...
# 原始字符串
data = '''FileObject(id='file-LipMxSsHd8foWAweucGYks', bytes=2563222, created_at=1738678393, filename='ar_attack_name.jsonl', object='file', purpose='batch', status='processed', status_details=None)
...
for i in range(1, len(lines), 2):
    ...
    match = re.match(r'Subset:\s*Global_MMLU/(\w+)\.csv\s+Attack_name:(.*?)\s+Batch Job ID:\s*(.*)', lines[i].strip())
    if match:
